...
| IddCx 1080p60 虚拟显示器 | 进行中 | 单屏驱动、构建/CAT/测试签名、`SwDeviceCreate` 生命周期与 `DN_STARTED` 校验均已实现；一体化包已在 `192.168.9.26` 成功暂存驱动并启动设备。活动 RDP 以 `PNP_VetoOutstandingOpen` 阻止显示栈刷新且 Windows 要求重启，故桌面拓扑中的 1080p60 显示器仍未验收 |
...
| Release 桌面构建与安装 | 已完成 | `npm run build` 生成桌面程序；`npm run build:installer` 生成包含 PeerSpan 驱动、测试证书、防火墙规则和 WebView2 离线运行时的一体化 NSIS 包，安装链路已在 `192.168.9.26` 通过 |
