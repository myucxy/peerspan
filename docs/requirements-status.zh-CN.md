...
| D3D11 帧获取和硬件编码 | 进行中 | IddCx 交换链线程已正确获取并释放 D3D11 帧；桌面端已实机完成 D3D11 NV12 纹理 → NVIDIA H.264 硬件编码 → Windows D3D11 解码 → NV12 帧闭环，覆盖异步超时和流格式重协商；IDD BGRA 纹理跨进程交接与 GPU 颜色转换仍待接入 |
