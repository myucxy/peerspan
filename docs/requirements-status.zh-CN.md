...
| D3D11 帧获取和硬件编码 | 进行中 | IddCx 交换链线程已正确获取并释放 D3D11 帧；桌面端已实机探测 D3D 11.1、D3D11-aware NVIDIA H.264 硬件编码器和 Windows H.264 硬件加速解码器，并在启动诊断中报告真实结果；IDD 纹理交接、颜色转换和实时编解码仍待接入 |
