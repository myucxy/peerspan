...
| 远端解码显示与输入回传 | 已完成 | 原生 Win32/D3D11 双缓冲交换链窗口、硬解 NV12 呈现、归一化鼠标、滚轮、扫描码键盘、`SendInput` 注入、可配置紧急释放和断连全键释放均已实现；硬编→硬解→`Present` 实机测试通过 |
...
当前开发机未安装 PeerSpan 测试驱动，且仓库不会自动修改测试签名或系统信任。因此代码、硬件编解码、共享纹理、D3D11 呈现和驱动构建已经完成验证；“真实 IddCx 帧跨两台电脑”和 Windows 10/11 驱动安装矩阵必须在允许安装测试驱动的专用机器上完成，不能用模拟画面替代。
