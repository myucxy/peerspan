...
局域网发现使用 `_peerspan._tcp.local.` mDNS 服务，桌面端每 2 秒刷新一次协议 v2 兼容节点。mDNS 的主服务端口为 TLS 控制端口 `37622`，TXT 记录另外发布配对端口 `37621`。配对引导使用 120 秒有效的六位代码和 SPAKE2 协商共享秘密，以 XChaCha20-Poly1305 保护身份交换，并用 Ed25519 签名验证持久设备身份；单次配对邀请最多允许 5 次尝试。发现测试和本机双端配对测试均已通过。

长期控制通道仅启用 TLS 1.3，ALPN 为 `peerspan-control/2`。客户端和服务端都使用本机长期 Ed25519 身份签发的自签名证书，并按配对时保存的 Ed25519 公钥固定证书 SPKI；服务端在每次握手时读取最新可信设备列表，因此未配对或已撤销的客户端会在 TLS 握手阶段被拒绝。TLS 建立后，协议 Hello 还会再次核对协议版本、设备 UUID、指纹和证书公钥，防止发现记录或应用层身份与证书错配。

当前控制会话支持 Display Offer/Decision、500 ms 心跳、往返延迟统计、主动 Session End 和双端清理。核心层只允许一个活动显示会话并限制状态合法迁移。媒体、虚拟显示器生命周期和输入链路尚未接通时会明确拒绝相应能力；已认证的显示会话保持 `Negotiating`，只有真实媒体开始传输后才能进入 `Streaming`。媒体通道仍需实现认证加密传输。
